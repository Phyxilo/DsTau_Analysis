...
vector<float> mel, mer, mpl, mpr, mtl, mtr;
vector<float> el, er, pl, pr, tl, tr;
       ...
       for(int x=1;x<=63;x++)
        {
            ...
            for (int i=pl1; i<=pl1+16;i++) //i nu e 1 si N e dat de file 
            {
                ...
                sprintf(filename, "PR2018_PD05_par/pl%03d_%03d/reco%02d_%06d_%06d/par_finealign/%02d_001.par",pl1,pl2,x,currentRow,currentColumn,i);
            ...
            for(int i=pl1;i<=pl1+15;i++) // calculezi toate limitele din subarea x din subvolume
            {
            ...
            sprintf(outfilenamep,"limits_PD05/limits_subarea_%0d_PD05_subvolume_%d_%d_Plastic.txt",x, pl1, pl2);
            cout << "Writting "<< outfilenamep<<endl;
            myfilep.open (outfilenamep);

            for(int i=pl1;i<=pl1+15;i++)
            {
            ...
            sprintf(outfilenamet,"limits_PD05/limits_subarea_%0d_PD05_subvolume_%d_%d_Tungsten.txt",x, pl1, pl2);
            ...
            sprintf(outfilenamee,"limits_PD05/limits_subarea_%0d_PD05_subvolume_%d_%d_Emulsion.txt",x, pl1, pl2);
...
*/
    ...
    ifstream fp;
    char Plastic[500];  
    sprintf(Plastic,"limits_PD05/limits_subarea_%0d_PD05_subvolume_%d_%d_Plastic.txt",subarea,plt,pl2);
    fp.open (Plastic);
    ...
    ifstream fe;
    char Emulsion[500];
    sprintf(Emulsion,"limits_PD05/limits_subarea_%0d_PD05_subvolume_%d_%d_Emulsion.txt",subarea,plt,pl2);
    fe.open (Emulsion);
    ...
    ifstream ft;
    char Tungsten[500];
    sprintf(Tungsten,"limits_PD05/limits_subarea_%0d_PD05_subvolume_%d_%d_Tungsten.txt",subarea,plt,pl2);
    ft.open (Tungsten); 
...
void ZVertex_aligned_vertices()
{   
    //All_limits(); //only once!!
    
    //h_align = new TH1F("h_all","h_all",300,0,6000);
    TH1F *h_align = new TH1F("h_all","h_all",2500,0,50000);
...
/*
 ...
 t3->AddText("Vertices in the depth of detector");
 t3->SetTextSize(0.06);
 t3->SetFillColor(0);
 t3->SetBorderSize(0);
 t3->Draw();
 c3->Print("ZVertex__v20220912_PD05_all_aligned_vertices.pdf");
