...
void VertexReduction()
{
  ...
  */
  ...
  par->Branch("global_parID",&par_globID,"global_parID/I");
  ...
  for (int subV = 0; subV < inFiles.size(); subV++)
  {
    ...
    Vtx_int_type = VtxTree->GetLeaf("intType");
    ...
    */
    ...
    Par_vID = ParTree->GetLeaf("vID");
    Par_flagp = ParTree->GetLeaf("flagp");
    Par_globID = ParTree->GetLeaf("global_parID");
...
void VtxCopy()
{
  ...
  vtx_int_type = Vtx_int_type->GetValue();
...
void ParCopy()
{
  ...
  par_flagp = Par_flagp->GetValue();
  par_globID = Par_globID->GetValue();
  //par_vID = Par_vID->GetValue();
