...
vector<pair<int, int>> ReadFile(char *inFile);
...
TFile *Dat2Root(string inputName, string outputName, string inputDS, string inputUS)
{
   ...
   */
   ...
   USPair = ReadFile(USChr);
   DSPair = ReadFile(DSChr);
   
   sort(USPair.begin(), USPair.end());
   sort(DSPair.begin(), DSPair.end());
   
   USPair.erase(unique(USPair.begin(), USPair.end(), firstEqual), USPair.end());
   DSPair.erase(unique(DSPair.begin(), DSPair.end(), firstEqual), DSPair.end());

   for (int i = 0; i < USPair.size(); i++)
   {
      int trid = USPair[i].first;
      int gtrid = USPair[i].second;

      UStrID.push_back(trid);
      USgtrID.push_back(gtrid);

      ptrk_US_trID = trid;
      ptrk_US_gtrID = gtrid;
      ptrk_US_plt_of_1seg = 0;
   ...
   {
      int trid = DSPair[i].first;
      int gtrid = DSPair[i].second;

      DStrID.push_back(trid);
      DSgtrID.push_back(gtrid);

      ptrk_DS_trID = trid;
      ptrk_DS_gtrID = gtrid;
      ptrk_DS_plt_of_1seg = 0;
      ...
      if(strcmp(type,"1ry_vtx") == 0)
      {
         ...
         if (vtx_n_1ry_parent_dmin_cut > 0)
         {
            ...
            for(int x = 0; x < paridVec.size(); x++)
            {
               ...
               if (it != UStrID.end())
               {
                  int index = it - UStrID.begin();
                  parC_id = paridVec[x];
                  
                  vtx_int_type = 1;
                  par_flagp = 1;

                  glob_parid = USgtrID[index];
               }
...
vector<pair<int, int>> ReadFile(char *inFile)
{
   ...
   for(int i = 0; i < numLines; i++)
   {
      ...
      trIDVec.push_back(make_pair(trID[i], globalTrID[i]));
   }
   fclose(fUS);
