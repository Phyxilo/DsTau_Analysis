...
TFile *Dat2Root(string inputName, string outputName, string inputDS, string inputUS, int dirIndex)
{
   ...
   */
   ...
   for (int segNum = 0; segNum <= 2; segNum++)
   {
      ...
      vector<pair<int, int>> USPair [15], DSPair [15];
      //vector<pair<int, int>> USPair, DSPair;
      //set<pair<int, int>> USPair, DSPair;

      vector<int> UStrID[15], USgtrID[15];
      vector<int> DStrID[15], DSgtrID[15];
      ...
      for (int areaInd = 0; areaInd < 15; areaInd++)
      {
         ...
         sprintf(USIn, "Pythia_Full_3Sigma/Area_%02d/Output/Merged/%s", areaInd+1, USChr);
         sprintf(DSIn, "Pythia_Full_3Sigma/Area_%02d/Output/Merged/%s", areaInd+1, DSChr);
