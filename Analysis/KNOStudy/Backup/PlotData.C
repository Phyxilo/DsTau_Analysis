...
void PlotData()
{
    ...
    // Save the plot
    canvas->SaveAs("ArticleHistogram.pdf");

    // Print the percentage errors for verification (optional)
    cout << "Percentage Errors (%):" << endl;
    for (int i = 0; i < dataSize; i++) {
        cout << "Bin " << i + 1 << " (" << ChargedTopology[i] << "): " << PercentageErrors[i] << "%" << endl;
    }

    cout << "\nHistogram Mean: " << hist->GetMean() << endl;
}